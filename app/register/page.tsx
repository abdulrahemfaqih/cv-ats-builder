    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          emailRedirectTo: `${redirectOrigin}/auth/callback?next=${encodeURIComponent(
            returnUrl
          )}`,
        },
      });
      ...
      // If user session is created immediately (no email verification required)
      if (data.session) {
        ...
        setSuccessMsg(
          "Pendaftaran berhasil! Silakan periksa inbox email Anda untuk memverifikasi akun."
        );
        setLoading(false);
      }
    } catch (err: unknown) {
      setErrorMsg(
        err instanceof Error ? err.message : "Terjadi kesalahan saat pendaftaran."
      );
      setLoading(false);
    }
  };
